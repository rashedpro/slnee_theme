...
class LoginManager:
	...
	def set_user_info(self, resume=False):
		...
		if not resume:
			...
			l=frappe.db.get_all("Employee",filters={"user_id":self.user})
			if len(l)>0:
				frappe.response["employee"]=l[0]["name"]
				shifts=frappe.get_all("Shift Assignment",fields=["*"],order_by="start_date desc")
				today=frappe.utils.getdate()
				start=None
				for s in shifts:
					if (s["start_date"] < today and not s["end_date"]) or (s["start_date"] < today and s["end_date"] > today ):
						frappe.response["shift_type"]=s["shift_type"]
						start=frappe.db.get_value("Shift Type",s["shift_type"],"start_time")
						end=frappe.db.get_value("Shift Type",s["shift_type"],"end_time")
						frappe.response["shift_start_time"]=str(start)[:-3]
						frappe.response["shift_end_time"]=str(end)[:-3]
						locations=[]
						loc=frappe.db.get_value("Shift Type",s["shift_type"],"allowed_locations")
						if loc:
							loc=json.loads(loc)
							for f in loc["features"]:
								if f["properties"]["point_type"]=='circle':
									geo=f["properties"]
									locations.append({"lat":f["geometry"]["coordinates"][1],"long":f["geometry"]["coordinates"][0],"radius":round(float(geo["radius"]),2)})
							frappe.response["allowed_location"]=locations
						else:
							frappe.response["allowed_location"]=[]
				if not start:
					frappe.response["allowed_location"]=[]
					frappe.response["shift_type"]=None
					frappe.response["shift_start_time"]=None
					frappe.response["shift_end_time"]=None
			else:
				frappe.response["employee"]=None
				frappe.response["shift_type"]=None
				frappe.response["shift_start_time"]=None
				frappe.response["shift_end_time"]=None
			frappe.response["company"]=frappe.defaults.get_user_default("Company")
			#frappe.response["allowed_location"]=[{'lat':24.7313448,'long':46.7876448,'radius':100}]
